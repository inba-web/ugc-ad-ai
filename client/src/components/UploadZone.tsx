...
const UploadZone = ({ label, file, onClear, onChange }: UploadZoneProps) => {
  return (
    <div className="relative-group">
      <div
        className={`relative h-64 rounded-2xl border-2 border-dashed transition-all durataion-300 flex flex-col items-center justify-center bg-white/2 p-6 ${file ? "border-violet-600/50 bg-violet-500/50" : "border-white/10 hover:border-violet-500/30 hover:bg-white/5"}`}
      >
        {file ? (
            <>
            ...
            <div className="absolute inset-0 flex items-center justify-center opacity">
                <button type="button" className="p-2 rounded-full bg-white/10 hover:bg-red-500/20 text-white hover:text-red-400 transition-colors" onClick={onClear}>
            ...
            <>
              <div className="w-16 h-16 rounded-full bg-white/5 flex items-center justify-center mb-4 group:scale-110 transition-transform duration-300">
