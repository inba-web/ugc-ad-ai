...
const ProjectCard = ({
  ...
  const handleDelete = async (id: string) => {
   ...
   const togglePublish = async (projectId: string) => {
    try {
      const token = await getToken();
      const {data} = await api.get(`/api/users/publish/${projectId}`, {
        headers: {Authorization: `Bearer ${token}`}
      } )
        ...
        >
              ...
              <div className="flex flex-col items-end w-32 text-sm">
                ...
                >
                  ...
                  {(gen.generatedVideo || gen.generatedImage) && (
                    <button className="w-full flex gap-2 items-center px-4 py-2 hover:bg-black10 cursor-pointer">
                      <Share2Icon size={14} onClick={() => navigator.share({url: gen.generatedVideo || gen.generatedImage, title: gen.productName, text:gen.productDescription})} /> Share
                    </button>
                  )}
          ...
          <div className="absolute right-3 bottom-3">
            ...
            <img
              ...
              alt="model"
              className="w-16 -16 object-cover rounded-full animate-float -ml-8"
        ...
        <div className="p-4">
          <div className="flex item-start justify-between gap-4">
            ...
            <div className="flex-1">
              ...
              <p>Created: {gen.createdAt ? new Date(gen.createdAt).toLocaleString() : 'Unknown'}</p>
              {gen.updatedAt && (
